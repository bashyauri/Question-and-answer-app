...
def get_current_user():
    ...
    if "user" in session:
        ...
        db = get_db()
        user_cur = db.execute(
            "select id, name,password,admin,expert from users where name = ?", [user]
        )
...
def question(question_id):
    ...
    question_cur = db.execute(
        ...
        on experts.id = questions.expert_id where questions.id = ?""",
        [question_id],
    )
...
def unanswered():
    ...
    questions_cur = db.execute(
        """select users.name,
                            ...
                            join users on users.id = questions.asked_by_id where answer is null and expert_id = ?""",
        [user["id"]],
    )
